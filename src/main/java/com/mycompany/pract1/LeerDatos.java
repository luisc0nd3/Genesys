...
public class LeerDatos {
  ...
  public static void main(String[] args) throws IOException {
    ...
    // 1. Preguntar al usuario cuantos registros quiere agregar Scanner or
    // BufferReader.

    // 2. llenar cada uno de los objetos ingresados por el usuario <PersonDTO> con
    // un ciclo.

    // 3. validar cada una de las propiedades del objeto <PersonDTO> Nota: validar
    // de golpe todos los objetos ingresados
    // por el usuario.

    // 4. imprimir el objeto señalando el dato incorrecto.
    /*
     * PersonDTO [name=Dato incorrecto, age=28, sex=H, email=Dato incorrecto, phone=null,
     * height=1.75, weights=105.5] PersonDTO [name=Irma Alicia, age=28, sex=H, email=Dato
     * incorrecto, phone=null, height=1.75, weights=105.5] PersonDTO [name=Mariano, age=28, sex=H,
     * email=Dato incorrecto, phone=null, height=1.75, weights=105.5]
     * 
     */
    ...
    //
    // people.get(i).setName( (validarLetras(people.get(i).getName())? people.get(i).getName():
    // "Dato incorrecto") );
    // people.get(i).setEmail( (validarLetras(people.get(i).getEmail())? people.get(i).getEmail():
    // "Dato incorrecto") );
    //
    //// people.get(i);
    ...
    for (int i = 0; i < count; i++) {
      ...
      /*
       * if (!validarLetras(user.getName())){ user.setName("Error"); nEr++; }
       */
      // user.setName((validarLetras(user.getName())? user.getName(): "Dato
      // incorrecto") );
      try {
        System.out.println("Edad: ");
        user.setAge(in.nextByte());
      } catch (NumberFormatException e) {
        System.out.println("Error ingresando dato");
      }
      System.out.println("Sexo");
      System.out.println("(M)asculino,(F)emenino, (N)o Definido: ");
